package com.github.kmu_wink.wink_official_page.domain.program.study.task;
...
public class TistoryParseTask {
    ...
    private void run() {
        ...
        for (int index = localLatestIndex + 1; index <= remoteLatestIndex; ++index) {
            try {
                ...
                Optional<String> image = firstImage(
                        document,
                        "#content > div > div.entry-content > div.contents_style"
                );
                content = content.replaceAll("[\\s\\n\\t]+", " ").trim().substring(0, Math.min(content.length(), 300));
    ...
    private int getRemoteLatestIndex() {
        ...
        Document latestDocument = Jsoup.connect(
                "https://cs-kookmin-club.tistory.com/category/WINK-%28Web%20%26%20App%29").get();
    ...
    private int getLocalLatestIndex() {
        ...
        return studyRepository.findTopByOrderByIndexDesc().orElseGet(() -> Study.builder().index(0).build()).getIndex();
    }

    private Optional<String> transferWinkCategory(Document document) {

        String category = Objects.requireNonNull(document.selectFirst("#content > div > div.hgroup > div.category"))
                .text();
